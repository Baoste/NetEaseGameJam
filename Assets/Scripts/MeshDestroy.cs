...
public class MeshDestroy : MonoBehaviour
{
    ...
    public List<PartMesh> DestroyMesh(int CutCascades, float angle = -1, float planeOffset = 0.05f, float forceMagnification = 0.1f)
    {
    ...
    private Plane BuildDiscLikePlane(Bounds bounds, int cascadeIndex, int partIndex, float angle, float planeOffset)
    {
        ...
        // 先在光盘平面里建立两个基向量
        Vector3 axisA = Vector3.Cross(up, Vector3.right);
        if (axisA.sqrMagnitude < 0.0001f)
            axisA = Vector3.Cross(up, Vector3.forward);
        axisA.Normalize();
        ...
        // 光盘平面上的放射方向
        Vector3 radialDir = (axisA * Mathf.Cos(rad) + axisB * Mathf.Sin(rad)).normalized;

        // “竖直切割平面”的法线
        Vector3 planeNormal = Vector3.Cross(radialDir, up).normalized;
        ...
        planeNormal = (tilt * planeNormal).normalized;
        ...
        point += radialDir * UnityEngine.Random.Range(-radiusHint * 0.15f, radiusHint * 0.15f);

        return new Plane(planeNormal, point);
    }
    ...
    public class PartMesh
    {
        ...
        public void MakeGameobject(MeshDestroy original)
        {
            ...
            //var dissolution = GameObject.AddComponent<DissolutionController>();
    ...
    //    Gizmos.DrawSphere(fractureCenter, 0.05f);
    //}
}